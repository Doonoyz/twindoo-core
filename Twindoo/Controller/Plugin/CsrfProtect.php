 ...
 * @version    Paper
 */
class Twindoo_Controller_Plugin_CsrfProtect extends Zend_Controller_Plugin_Abstract {
	/**
	 * Session storage
	 *
	...
	/**
	 * The token, set by _initializeToken
	 *
	 * @var string
	 */
	protected $_token = '';
	/**
	 * Active the protection or not
	 *
	 * @var string
	 */
	...
	public function dispatchLoopShutdown() {
		if ($this->_enabled) {
			...
			$element = sprintf ( '<input type="hidden" name="%s" value="%s" />', $this->_keyName, $token );
	...
	/**
	 * Set the enabled state for the plugin
	 *
	 * @param bool $bool Enable state
	 */
